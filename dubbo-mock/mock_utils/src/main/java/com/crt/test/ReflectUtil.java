...
public class ReflectUtil {
	...
	try {
		...
		if (mapValue != null) {
			...
			throw new RuntimeException("不一致的属性类型 ： [fieldName=" + fieldName + ",mapValueType="
				+ mapVClass.getName() + ",fieldType=" + fieldType.getName() + "]");
		    }
		}
	    }
	    return bean;
	...
	if (bean instanceof Map) {
		...
		if (field == null) {
		    throw new RuntimeException(
			    "There is no field named as '" + fieldName + "' in Class:" + zlass.getName());
		} else {
